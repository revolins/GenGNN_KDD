...
def create_folders(args):
    # Check if hierarchical directory is being used (ablation study)
    hier_dir = getattr(args, 'hier_dir', None)

    if hier_dir:
        # For ablation studies, create subdirectories within the organized directory
        try:
            os.makedirs(os.path.join(hier_dir, "graphs"), exist_ok=True)
            os.makedirs(os.path.join(hier_dir, "chains"), exist_ok=True)
            os.makedirs(os.path.join(hier_dir, "outputs"), exist_ok=True)
            if args.train.save_model:
                os.makedirs(os.path.join(hier_dir, "checkpoints"), exist_ok=True)
        except OSError:
            pass
        return

    # Default behavior for non-ablation studies
    general_name = getattr(args.general, 'name', 'debug') if hasattr(args, 'general') else 'debug'

    try:
        if args.train.save_model:
            os.makedirs(f"checkpoints/{general_name}", exist_ok=True)
        os.makedirs(f"graphs/{general_name}", exist_ok=True)
        os.makedirs(f"chains/{general_name}", exist_ok=True)
    except OSError:
        pass
...
class PlaceHolder:
    ...
    def mask(self, node_mask, collapse=False):
        ...
        # Accept node_mask as [B,N] or [B,N,1]
        if node_mask.dim() == 3:
            node_mask_2d = node_mask.squeeze(-1)  # [B,N]
        else:
            node_mask_2d = node_mask              # [B,N]

        # bool/float both ok
        if node_mask_2d.dtype == torch.bool:
            nm = node_mask_2d
            nm_f = nm.to(self.X.dtype) if self.X is not None else None
        else:
            nm_f = node_mask_2d.to(self.X.dtype) if self.X is not None else node_mask_2d
            nm = node_mask_2d > 0.5

        x_mask = (nm_f if nm_f is not None else nm.to(torch.float32)).unsqueeze(-1)  # [B,N,1]
        ...
        if collapse:
            ...
            # print("x_mask size: ", x_mask.size(), flush=True)
            self.X = self.X * x_mask
            self.E = self.E * e_mask1 * e_mask2
            # Ensure E is symmetric due to floating point issues
            self.E = 0.5 * (self.E + torch.transpose(self.E, 1, 2))
