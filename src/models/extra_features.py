...
class ExtraFeatures:
    def __init__(self, extra_features_type, rrwp_steps, dataset_info, ablation_flags=None):
    ...
    def __call__(self, noisy_data):
        n = noisy_data["node_mask"].sum(dim=1).unsqueeze(1).float() / self.max_n_nodes
        # Ensure n is always a tensor, not a scalar
        if not isinstance(n, torch.Tensor):
            n = torch.tensor(n, device=noisy_data["node_mask"].device, dtype=torch.float32)
        if n.dim() == 0:
            n = n.unsqueeze(0).unsqueeze(0)

        # Make a copy of n to ensure it doesn't get overwritten by local variables
        n_original = n.clone()
        x_cycles, y_cycles = self.ncycles(noisy_data)  # (bs, n_cycles)
        # print("n : ", n.size())
        # print("x_cycles: ", x_cycles.size())
        # print("y_cycles: ", y_cycles.size())

        if self.features_type == "cycles":
            E = noisy_data["E_t"]
            extra_edge_attr = torch.zeros((*E.shape[:-1], 0)).type_as(E)
            return utils.PlaceHolder(
                X=x_cycles, E=extra_edge_attr, y=torch.hstack((n_original, y_cycles))
            )
            ...
            return utils.PlaceHolder(
                X=x_cycles,
                E=extra_edge_attr,
                y=torch.hstack((n_original, y_cycles)),
            )

        elif self.features_type == "rrwp":
            # print('noisy data E: ', noisy_data["E_t"].size())
            E = noisy_data["E_t"].float()[..., 1:].sum(-1)  # bs, n, n # OLD
            # print("E current size: ", E.size())

            if self.ablation_flags["use_rrwp_edge_features"]:
                rrwp_edge_attr = self.RRWP(E, k=self.rrwp_steps)
            else:
                bs, n, _ = E.shape
                rrwp_edge_attr = E.new_zeros(bs, n, n, self.rrwp_steps)

            if self.ablation_flags["use_rrwp_node_features"]:
                diag_index = torch.arange(rrwp_edge_attr.shape[1])
                rrwp_node_attr = rrwp_edge_attr[:, diag_index, diag_index, :]
            else:
                bs, n, _ = E.shape
                if self.ablation_flags["use_rrwp_edge_features"]:
                    rrwp_node_attr = rrwp_edge_attr.new_zeros(bs, n, self.rrwp_steps)
                else:
                    rrwp_node_attr = E.new_zeros(bs, n, self.rrwp_steps)
            ...
            # Ensure n is always a tensor for consistency
            if not isinstance(n, torch.Tensor):
                n = torch.tensor(n, device=noisy_data["node_mask"].device, dtype=torch.float32)
            if n.dim() == 0:
                n = n.unsqueeze(0).unsqueeze(0)
            ...
            return utils.PlaceHolder(
                ...
                E=extra_edge_attr,
                y=torch.hstack((n_original, y_cycles, n_components, batched_eigenvalues)),
            )
